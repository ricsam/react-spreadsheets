      ...
      <div className="rsp-workbook-body">
        <Spreadsheet
          ...
          customCellStyle={(cell) => {
            ...
            // Excel-like table styling using CSS variables for dark mode support
            ...
            if (isHeaderRow) {
              ...
              style.borderBottom = '2px solid var(--table-header-border)';
            } else {
              // Data rows - alternating background
              const dataRowIndex = cell.rowIndex - tableInfo.start.rowIndex - 1;
              if (dataRowIndex % 2 === 0) {
                style.backgroundColor = 'var(--table-even-row-bg)';
              } else {
                style.backgroundColor = 'var(--table-odd-row-bg)';
              }
            }

            // Border styling
            if (isFirstColumn) {
              style.borderLeft = '2px solid var(--table-border-color)';
            }
            if (isLastColumn) {
              style.borderRight = '2px solid var(--table-border-color)';
            }
            if (isHeaderRow) {
              style.borderTop = '2px solid var(--table-border-color)';
            }
            if (isLastRow) {
              style.borderBottom = '2px solid var(--table-border-color)';
            }
